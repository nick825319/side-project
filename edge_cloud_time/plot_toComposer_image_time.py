...
fdata = np.loadtxt('measure_toComposer_image_time.txt', delimiter = ' ')
...
ax.set_ylabel('image_trans(miil_sec)')
ax.set_title('composer_transfer_image_time')

fdata = fdata*1000
average = sum(fdata) / len(fdata)
plt.yticks(np.arange(0,max(fdata)+average*0.1, 3))
...
plt.savefig('./measure_toComposer_image_time.pdf')
plt.show()
