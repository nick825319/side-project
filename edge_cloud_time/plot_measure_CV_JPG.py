...
fdata = np.loadtxt('measure_CV_JPG.txt', delimiter = ' ')
...
plt.yticks(np.arange(0,average+average*0.5, 5))
plt.xticks(np.arange(0, round(len(fdata)+len(fdata)*0.01), len(fdata)/20), rotation=60)
ax.plot(fdata, linestyle='-')
ax.set_ylim(ymin=0,ymax=average+average*0.5)

plt.savefig('./measure_CV_JPG.pdf')
plt.show()
